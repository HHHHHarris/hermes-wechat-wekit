...
Handling is deliberately minimal: if `type != 1` **and** `Content` is empty, the text becomes a placeholder such as `[image message]`. If a non-text type carries content (type 49 typically carries XML), that content is passed through as-is. Every inbound event is constructed as `MessageType.TEXT` regardless of the WeChat type — **this adapter does not download, decode, or re-host inbound media**. The agent never sees the actual picture, voice clip, or file.
